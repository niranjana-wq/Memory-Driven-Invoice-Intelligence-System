...
import { Memory, VendorMemory, CorrectionMemory, ResolutionMemory, MemoryQuery } from '../types';
...
export class MemoryDatabase {
  ...
  async saveMemory(memory: Memory): Promise<void> {
    ...
    switch (memory.type) {
      ...
      case 'resolution':
        const resMem = memory as ResolutionMemory;
        await this.dbRun(`
  ...
  async updateMemoryConfidence(memoryId: string, newConfidence: number): Promise<void> {
    const now = new Date().toISOString();
    await this.dbRun(`
      UPDATE memories 
